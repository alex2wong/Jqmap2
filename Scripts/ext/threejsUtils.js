...
// camPosition response to mapCenter&mapZoom, lookAt response to mapPitch&bearing
ThreeShp.init = function(canvas, map){
  // set the canvas width and length
  this.c = new THREE.PerspectiveCamera(41, canvas.width/canvas.height, 1, 100000);
  this.r.setSize(canvas.width, canvas.height);  
  // THREE data struct.. Scene.add(Mesh)/Object3D
  // 照相机在三维空间中的位置。 
  this.c.position.z = 150;
  this.c.position.y = map.getCenter().lat;
  this.c.position.x = map.getCenter().lng;
...
loader.load('../../Asset/optimer_regular.typeface.json', function(res) {
    var font = res;
    displayObjName(font);
});
...
var texture = null;
// new THREE.TextureLoader().load('img/grass.jpg');
