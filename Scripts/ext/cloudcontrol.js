...
// Mixin/Implement ?? copy attri and method from Control to Cloud !!
...
{	if (this.getMap()) 
		// remove previous map instance's event listener.
	{	this.getMap().un('postcompose', this.drawCloud_, this);
	}
	// control bind to new map instance.
...
{	options = options || {};
	...
	// which determine the cloud move direction and speed.
	this.wind = { angle: a, cos: Math.cos(a), sin: Math.sin(a), speed: speed };
}

/**
*	@private. input postcompose evt
*/
...
//	var rot = view.getRotation();
	...
	// Go! this.cloud is the Image instace, eg: width 100px
	...
	// WTF <<    canvas.width*height/cloud.width/height means the area ratio
